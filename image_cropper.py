...
denoise = input("Denoise (Y/n): ").lower() != "n"

# Apply face detection (optional)
detect_faces = input("Detect faces (Y/n): ").lower() != "n"

# Initialize a cascade classifier for face detection (optional)
face_cascade = cv2.CascadeClassifier("haarcascade_frontalface_default.xml") if detect_faces else None
...
for file in os.listdir(dir_path):
	if file.lower().endswith((".jpg", ".jpeg", ".webp", ".png")):
		...
		if detect_faces:
			...
			# Adjust the cropping coordinates if they go beyond the image boundaries
			...
			# Calculate the cropping region
			crop_x1, crop_y1 = 0, 0
			crop_x2, crop_y2 = crop_width, crop_height
